...
shared actor class Cosmicrafts(collectionOwner: TypesICRC7.Account, init: TypesICRC7.CollectionInitArgs) = Self {
// Types
...
// User-Specific Missions
    ...
    public func createUserMission(user: PlayerId): async (Bool, Text, Nat) {
        ...
        // Create and assign hourly mission
        if (shuffledHourlyIndices.size() == 0 or currentHourlyIndex >= shuffledHourlyIndices.size()) {
            await initializeShuffledHourlyMissions();
        };
        let hourlyResult = await createUserSpecificMission(user, MissionOptions.hourlyMissions, shuffledHourlyIndices, currentHourlyIndex, ONE_HOUR);
        currentHourlyIndex += 1;

        // Create and assign daily mission
        if (shuffledDailyIndices.size() == 0 or currentDailyIndex >= shuffledDailyIndices.size()) {
            await initializeShuffledDailyMissions();
        };
        let _dailyResult = await createUserSpecificMission(user, MissionOptions.dailyMissions, shuffledDailyIndices, currentDailyIndex, ONE_DAY);
        currentDailyIndex += 1;

        // Create and assign weekly mission
        if (shuffledWeeklyIndices.size() == 0 or currentWeeklyIndex >= shuffledWeeklyIndices.size()) {
            await initializeShuffledWeeklyMissions();
        };
        let _weeklyResult = await createUserSpecificMission(user, MissionOptions.weeklyMissions, shuffledWeeklyIndices, currentWeeklyIndex, ONE_WEEK);
        currentWeeklyIndex += 1;
    ...
    public shared ({ caller }) func getUserMissions(): async [MissionsUser] {
        ...
        // If no active missions, create new user missions
        if (activeMissions.size() == 0) {
            Debug.print("[getUserMissions] No active missions found, creating new missions for the user.");
            ...
            // After creating new missions, reassign and search again
            await assignUserMissions(caller);
            activeMissions := await searchActiveUserMissions(caller);
        };
...
//--
// ICRC7
    private stable var owner: TypesICRC7.Account = collectionOwner;
    ...
    private stable var royaltyRecipient: ?TypesICRC7.Account = init.royaltyRecipient;
    ...
    public func get_transactions(getTransactionsArgs: TypesICRC7.GetTransactionsArgs): async TypesICRC7.GetTransactionsResult {
    ...
    public shared(msg) func upgradeNFT(nftID: TokenID): async (Bool, Text) {
        ...
        let transfer: TypesICRC1.TransferResult = await shards.icrc1_pay_for_transaction(_transactionsArgs, msg.caller);
        switch (transfer) {
            case (#Err(_e)) {
    ...
    public shared({ caller }) func openChest(chestID: Nat): async (Bool, Text) {
        ...
        switch (burnResult) {
            case null {
                ...
                var _tokensResults = shardsResult # ", " # fluxResult;
